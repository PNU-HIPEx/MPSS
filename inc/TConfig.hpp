...
namespace KEI {
	...
	public: // 외부에서 사용하는 함수
		...
		std::vector<std::string> getConfigTitleSet() const;
	private:
	...
	inline std::vector<std::string> TConfigFile::getConfigTitleSet() const {
		std::vector<std::string> titleSet;
		for ( const auto& config : mConfigSet ) {
			titleSet.push_back(config.getTitle());
		}
		return titleSet;
	}
